...
/**
 * accept two args
 * @param number >> formatedNmuber
 * @param iso2 >> iso2
 */
export const isValidNumber = PhoneNumber.isValidNumber;

export const getISO2fromNumber=(value)=>PhoneNumber.getCountryCodeOfNumber(value)
...
export default class PhoneInput extends Component {
  ...
  constructor(props, context) {
    ...
    const countryData = props.value?PhoneNumber.getCountryDataByCode(getISO2fromNumber(props.value)): PhoneNumber.getCountryDataByCode(initialCountry);
    console.log('the value>>>>',props.value)
    this.state = {
      iso2: props.value?getISO2fromNumber(props.value):PhoneNumber.getCountryDataByCode(initialCountry),
      disabled,
  ...
  getValue() {
    return this.state.formattedNumber.replace(/\s/g, "").replace(/-/g, "");
  }
    ...
    if (this.state.iso2 !== iso2) {
      ...
      if (countryData) {
        this.setState(
          ...
          () => {
            this.updateFlagAndFormatNumber(`+${countryData.dialCode}`,null,iso2);
  ...
  format(text) {
    return this.props.autoFormat && text!==this.getDialCode()
      ? PhoneNumber.format(text, this.state.iso2)
      : text;
  }

  updateFlagAndFormatNumber(number, actionAfterSetState = null,newISO=undefined) {
    const { allowZeroAfterCountryCode } = this.props;
    const countryCode = this.getCountryCode();
    const plainNumber=`${number}`.replace(/-/g, '')
    .replace(/ /g, '').replace(`+${countryCode}`,'')
      ...
      if (number) {
        ...
        formattedPhoneNumber = allowZeroAfterCountryCode
          ...
          : this.possiblyEliminateZeroAfterCountryCode(formattedPhoneNumber);
           iso2 = newISO || PhoneNumber.getCountryCodeOfNumber(formattedPhoneNumber);
      }
      this.setState(
        {
          iso2,
...
export const PhoneModalPicker = ModalPickerImage;
